<?php
	...
	$pageName = "Model";
	$pageURL = 'model-master.php';
	$addURL = 'model-add.php';
	$deleteURL = 'model-delete.php';
...
<?php
						...
						while($row = $admin->fetch($results)){
							$manufacturerDetails = $admin->getUniqueManufacturerById($row['manufacturer_id']);
?>
							<tr>
								<td><?php echo $x++; ?></td>
								<td><?php echo $row['model_name']; ?></td>
								<td><?php echo $manufacturerDetails['manufacturer_name']; ?></td>
								<td>
								<?php
									if($row['active'] == '0') {
								...
								?>
									<a class="" href="model-status.php?id=<?php echo $row['id']; ?>" onclick="return confirm('<?php echo $active_msg;?>');" title="Click to change model status."><?php echo $active_status; ?></a>
								</td>
								<td>
