<?php
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$manufacturer_name 		= trim($admin->escape_string($admin->strip_all($_POST['manufacturer_name'])));
			
			if(empty($manufacturer_name)){
				header("location:".$pageURL."?registerfail&msg=Please select a Manufacturer Name");
				exit();
			} else {
				//add to database
				$result = $admin->addManufacturer($_POST,$_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueManufacturerById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			...
			if(empty($manufacturer_name)){
				header("location:".$pageURL."?updatefail&msg=Please enter a Manufacturer name&edit&id=".$id);
				exit();
			}else {
				//update to database
				$result = $admin->updateManufacturer($_POST,$_FILES);
...
<head>
	...
	<title><?php echo TITLE ?> | Add Manufacturer</title>
			...
			<form role="form" action="" method="post" id="form" enctype="multipart/form-data">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h6 class="panel-title"><i class="icon-library"></i>Manufacturer Details</h6>
					</div>
						...
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									...
									<input type="text" class="form-control" required="required" name="manufacturer_name" id="manufacturer_name" value="<?php if(isset($_GET['edit'])){ echo $data['manufacturer_name']; }?>"/>
								</div>
