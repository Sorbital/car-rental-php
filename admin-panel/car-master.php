...
<head>
	...
	<title><?php echo TITLE; ?> | Car Master</title>
...
<?php
						...
						while($row = $admin->fetch($results)){
							$modelDetails = $admin->getUniqueModelById($row['model_id']);
?>
							<tr>
								<td><?php echo $x++; ?></td>
								<td><?php echo $modelDetails['model_name']; ?></td>
								...
								?>
									<a class="" href="car-status.php?id=<?php echo $row['id']; ?>" onclick="return confirm('<?php echo $active_msg;?>');" title="Click to change car status."><?php echo $active_status; ?></a>
								</td>
								<td>
